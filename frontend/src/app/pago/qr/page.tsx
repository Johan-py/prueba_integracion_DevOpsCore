...
export default function PagoQRPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-sans">Cargando pago...</div>;
  ...
  if (error || !payment) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center font-sans p-4">
        ...
        <p className="text-stone-500 text-sm mb-6 text-center">Asegúrate de que en Prisma Studio el Usuario 1 tenga un registro PENDIENTE.</p>
        <button onClick={() => window.location.reload()} className="bg-stone-800 text-white px-6 py-2 rounded-lg font-medium">
          Verificar de nuevo
        </button>
      </div>
    );
