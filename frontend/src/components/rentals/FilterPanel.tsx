...
const FilterSection: React.FC<FilterSectionProps> = ({ title, data, logic, itemLabel }) => {
  return (
    <section>
      ...
      <div className={`flex flex-col gap-1.5 mt-1 ${logic.viewLevel > 2 ? 'max-h-60 overflow-y-auto pr-2' : ''}`}>
        {logic.visibleData.map((item: FilterItem) => (
          <div key={item.name} className="flex justify-between items-center gap-3 group cursor-pointer transition-all">
  ...
  if (loading) {
    return (
      <div className="w-full lg:w-80 bg-white p-8 rounded-3xl lg:rounded-2xl border border-gray-100 shadow-sm lg:shadow-[0_10px_40px_rgba(0,0,0,0.06)] mb-8 lg:sticky lg:top-20 shrink-0 flex items-center justify-center">
        <span className="text-gray-500 italic font-inter font-medium text-sm animate-pulse">Sincronizando filtros...</span>
      </div>
    );
  }
    ...
    return (
      <div className="w-full lg:w-80 bg-white p-8 rounded-3xl lg:rounded-2xl border border-gray-100 shadow-sm lg:shadow-[0_10px_40px_rgba(0,0,0,0.06)] mb-8 lg:sticky lg:top-20 shrink-0 flex flex-col items-center justify-center text-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm font-inter mb-4">No pudimos cargar los filtros en este momento.</p>
          <button 
  ...
  const FilterHeader = () => (
    <div className="flex items-center justify-between mb-6 border-b border-gray-800 pb-3">
      ...
      <div className="flex items-center gap-4">
        <button 
          onClick={() => { setSortType('name'); toggleGlobalSort(); }} 
        ...
        <button 
          onClick={() => { setSortType('count'); toggleGlobalSort(); }} 
    ...
    <>
      ...
      <div className="lg:hidden w-full mb-8">
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 px-2 scrollbar-hide">
          <button onClick={() => setMobileTab('alquiler')} className={`flex-shrink-0 px-6 py-2.5 rounded-2xl text-sm font-bold font-inter transition-all ${mobileTab === 'alquiler' ? 'bg-orange-500 text-white shadow-lg shadow-orange-200' : 'bg-gray-100 text-gray-500'}`}>
            Alquileres
          </button>
          <button onClick={() => setMobileTab('venta')} className={`flex-shrink-0 px-6 py-2.5 rounded-2xl text-sm font-bold font-inter transition-all ${mobileTab === 'venta' ? 'bg-orange-500 text-white shadow-lg shadow-orange-200' : 'bg-gray-100 text-gray-500'}`}>
            En Venta
          </button>
