...
export default function TourGuiado() {
  ...
  // ─── Tooltip height + highlight re-measure on resize ──────────────────────
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const isVisible =
        rect.width > 0 && rect.height > 0 && rect.top < vh && rect.bottom > 0;
      if (isVisible) setHighlight(rect);
    };
  ...
  useEffect(() => {
    ...
    const tryFind = () => {
      ...
      if (attempts < maxAttempts) {
        ...
        if (!step.required) setCurrentStep((prev) => prev + 1);
